...
def heatmap_table(data, items, color="Oranges", vmin=None, vmax=None, dest_path=None, cbar_kws=None):
    ...
    sns.heatmap(df, annot=True, fmt=".4f", cmap=color, linewidths=1, cbar=True, vmin=vmin, vmax=vmax, cbar_kws=cbar_kws)
    plt.show()
...
def tsne_plot(embeddings, labels, keys, data_paths):
    ...
    dest_folder = "script/figures/output/embeddings"
