...
class PassGAN(Model):
    ...
    def prepare_model(self):
        ...
        self.checkpoint_dir = os.path.join("checkpoints", "PassGAN", self.train_hash)
    ...
    def train_discriminator(self, real_data):
        ...
        disc_cost, gradient_penalty = self.compute_disc_wgangp_loss(fake_data, real_data, disc_real, disc_fake,
                                                                    self.LAMBDA)
        disc_cost.backward()
    ...
    def build_model(self, mode):
        ...
        self.Generator = Generator(x_size, self.dict_size, layer_dim=layer_dim, is_training=is_training).to(self.device)

        self.G_vars = [p for p in self.Generator.parameters() if p.requires_grad]

        if is_training:
            self.Discriminator = Discriminator(x_size, self.dict_size, layer_dim=layer_dim, is_training=is_training).to(
                self.device)
