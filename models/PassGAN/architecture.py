...
class ResidualBlock(nn.Module):
    def __init__(self, layer_dim):
        ...
        self.res_block = nn.Sequential(
            nn.ReLU(),
            nn.Conv1d(in_channels=layer_dim, out_channels=layer_dim, kernel_size=5, padding='same'),
            nn.ReLU(),
            nn.Conv1d(in_channels=layer_dim, out_channels=layer_dim, kernel_size=5, padding='same'),
        )
...
class Generator(nn.Module):
    def __init__(self, max_length, n_chars, layer_dim=128, is_training=False):
...
class Discriminator(nn.Module):
    def __init__(self, max_length, dict_size, layer_dim=128, is_training=False):
